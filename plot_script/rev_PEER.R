...
age_cor_res$padj<-p.adjust(age_cor_res$p)
age_cor_res$sig<-ifelse(test = age_cor_res$padj<0.05,yes = "Sig.",no = "N.S.")

library(ggrepel)
age_cor_res$v<--log10(age_cor_res$padj)
...
ggscatter(age_cor_res,x = "beta",y="v",
  ...
  xlim(c(-240,240))+labs(title = "Association between age and count of losing correlations 
                         adjusted by PEER")
