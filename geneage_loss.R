...
files<-list.files("./Geneage/")
geneage_res<-data.frame(tissue = str_split(files,pattern = "gene",simplify = T)[,1],
                        cor=0,p=-1)

for(i in 1:26){
  load(paste0("./Geneage/",files[i]))
...
geneage_res$sig<-ifelse(test = geneage_res$p<0.05,yes = "Sig.",no = "N.S.")
